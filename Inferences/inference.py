...
def load_model_and_args(checkpoint_path, device='cpu'):
    ...
    if isinstance(args, dict):
        ...
        # Convert to namespace for easier access later
        args = Namespace(**args)
    else:
        model_type = args.model
        hidden_size = args.hidden_size
    ...
    if model_type == 'lstm':
        ...
        # Extract num_users from checkpoint
        num_users = checkpoint.get('num_users', None)
        if num_users is None and 'user_embedding.weight' in checkpoint['model_state_dict']:
            num_users = checkpoint['model_state_dict']['user_embedding.weight'].shape[0]
        
        # Infer Lag-Llama specific params from checkpoint weights if not in args
        d_model = 128  # Default
        if 'input_projection.weight' in checkpoint['model_state_dict']:
            d_model = checkpoint['model_state_dict']['input_projection.weight'].shape[0]
...
def predict_single(model, speed, altitude, gender, original_length, model_type, userId=None, device='cpu'):
    ...
    with torch.no_grad():
        ...
        elif model_type in ['lstm_embeddings', 'lag_llama']:
            if userId is None:
                raise ValueError(f"userId required for {model_type} model")
