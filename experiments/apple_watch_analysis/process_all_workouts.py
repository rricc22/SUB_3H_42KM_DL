...
def main():
    ...
    existing_csvs = len(list(output_dir.glob("*_processed.csv")))
    print(f"   ✓ {existing_csvs} CSV files already exist (will be skipped)")
    ...
    print("   (Existing CSV files will be skipped)")
    print("=" * 80)
    ...
    print(f"Newly processed: {successful} ({100*successful/len(workouts_with_gpx):.1f}%)")
    print(f"Skipped (already existed): {skipped} ({100*skipped/len(workouts_with_gpx):.1f}%)")
    print(f"Failed: {failed} ({100*failed/len(workouts_with_gpx):.1f}%)")
    print(f"Total successful: {successful + skipped} ({100*(successful+skipped)/len(workouts_with_gpx):.1f}%)")
    
    if successful_results:
        # Calculate aggregate statistics for newly processed workouts
        ...
        print(f"\n--- Heart Rate Quality (Newly Processed) ---")
        ...
        print(f"\n--- Temporal Distribution (Newly Processed) ---")
