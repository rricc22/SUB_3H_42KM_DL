...
Freeze layers 0, 1 and train layers 2, 3 + FC
Loads from Stage 1 best checkpoint
"""
...
class Trainer:
    """Training manager for Stage 2 fine-tuning"""
    ...
    def __init__(self, config):
        ...
        # Generate session timestamp (used for all checkpoints in this training session)
        self.session_timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        
        print(f"\n{'='*60}")
        print(f"STAGE 2 FINE-TUNING TRAINER INITIALIZED")
        print(f"{'='*60}")
    ...
    def save_checkpoint(self, epoch, is_best=False):
        """Save checkpoint with session timestamp (consistent across all saves)"""
        checkpoint = {
            'epoch': epoch,
        ...
        # Use session timestamp (same for entire training run)
        latest_path = self.checkpoint_dir / f'latest_model_{self.session_timestamp}.pt'
        torch.save(checkpoint, latest_path)
        
        # Save best with same session timestamp
        if is_best:
            best_path = self.checkpoint_dir / f'best_model_{self.session_timestamp}.pt'
...
def main():
    ...
    print(f"\nBest model: {config['checkpoint_dir']}/best_model_{trainer.session_timestamp}.pt")
    print(f"Val loss: {trainer.best_val_loss:.4f}")
    print(f"\nTo evaluate: python finetune/evaluate.py --stage 2")
